...
#Making the document
df = pd.read_parquet("../outputs/trades_baseline_bestcase.parquet")
pdf = FPDF()
pdf.add_page()
...
pdf.image('grafics/equity_curve.png', x = 10, w = 200)
pdf.ln(5)
...
pdf.cell(0,10, text = f"streak_loss: {rm.streak_losses(df)}", new_x='LMARGIN', new_y='NEXT')
pdf.image('grafics/profit_per_trade.png', x = 10, w = 150)
pdf.ln(5)
...
pdf.image('grafics/profit_weekday.png', x = 10, w = 200)
...
pdf.output('../report/results/baseline_bestcase_report.pdf')